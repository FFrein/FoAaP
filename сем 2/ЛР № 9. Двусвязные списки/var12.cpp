...
//-----------------------------------------------------------
void deleteX(char name[NAME_SIZE], Address** phead, Address** plast) { // Удаление имени {	struct Address *t = *phead;	
	Address* t = *phead;
	while (t)
	{
		if (!strcmp(name, t->name))  break;
